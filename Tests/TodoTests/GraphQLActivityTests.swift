...
@Test func graphqlActivityQueryAliasesEveryRepoAndReason() {
    ...
    // 2 repos × 3 searches, in stable per-repo order.
    #expect(aliases.count == 6)
    #expect(aliases.map(\.alias) == ["m0", "a0", "r0", "m1", "a1", "r1"])
    #expect(aliases[0].reason == .mention)
    #expect(aliases[1].reason == .assigned)
    #expect(aliases[2].reason == .reviewRequested)
    #expect(aliases[3].repo.name == "r2")

    // Every search is repo-scoped and aliased into the single document.
    #expect(query.contains(#"m0: search(query: "repo:some-org/r1 is:issue mentions:kalle", type: ISSUE, first: 50)"#))
    #expect(query.contains("repo:some-org/r1 assignee:kalle"))
    #expect(query.contains("repo:some-org/r2 is:pr is:open review-requested:kalle"))
    ...
    #expect(query.components(separatedBy: "stateReason").count == 7) // 6 Issue fragments + tail
